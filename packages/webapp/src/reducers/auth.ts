 ...
 * @returns {void} This function does not return anything.
 */
export const redirectHandler = () => (dispatch: any, getState: any) => {
  const { selectedAction } = getState().openai;
  if (selectedAction && selectedAction.value) {
    setTimeout(() => {
      Router.push(`/tools/${selectedAction.value}`);
    }, 500);
  } else {
    Router.push('/');
  }
...
export const createUserCallBack =
  (token: string, data: any) => (dispatch: any) => {
    ...
    const profile: IProfile = { lastLogin: new Date(), totalTrailUsed: 0 };
